...
// ========================================================================
    ...
    if (re_check_labeled.test(text)) {
        ...
        try {
            ...
            try {
                // *** KORREKTUR: \s+ durch [ \t]+ ersetzt, um Zeilenumbrüche zu verhindern ***
                ...
                // Job-Titel
                var re_job_title = /(Geschäftsführerin|Geschäftsführung|Geschäftsführer|Inhaberin|Inhaber|Vorstand)/gim;
        ...
        // ========================================================================
        // START: NEUE ADRESS-LOGIK (BASIEREND AUF PLZ-LISTE)
        // ========================================================================
        ...
        try {
            ...
            var full_plz_ort = match_plz_ort[0].trim();
        ...
        if (postleitzahl && ort) { // Nur ausführen, wenn wir einen Anker haben
            try {
                // Suche nach einer Zeile, die direkt VOR der PLZ/Ort-Zeile steht
                // und typischerweise auf eine Hausnummer endet.
                var re_strasse_context = new RegExp(
                    // $1: Die Straße (alles bis zur Hausnummer)
                    // \s+: Leerzeichen
                    // \d+[a-zA-Z]?: Hausnummer
                    // \s*\n+: Zeilenumbruch
                    // (?=...): Positive Lookahead - stellt sicher, dass danach der gefundene Ort folgt
                    `(.+\\s\\d+[a-zA-Z]?)\\s*\\n+(?=${postleitzahl}\\s+${escapeRegex(ort)})`, 
                    "gimu" // Multiline und Unicode sind wichtig
                );
        ...
        // Telefon
        try {
            var re_telefon = /(?:Tel|Fon|Telefon)[\D\s]*((?!(?:15|16|17))(?:\+|0|0049).*\d{1,14})$/gim;
            telefon = re_telefon.exec(text)[1];
